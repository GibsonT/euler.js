...
function checkFactors(factor) {
  ...
  // We're using the square root of the number (rounded up) to reduce the
  // number of values to check, which speeds up the runtime of the function
  // significantly.
  //
  // This is based on the Sieve of Eratosthenes algorithm.
  for (let i = 2; i <= Math.ceil(Math.sqrt(factor)); i++) {
    ...
    // We're starting from the bottom up, so if factor / i is prime,
    // then, logically, factor / i is the greatest prime factor.
    if (isPrime(factor / i)) {
      return factor / i;
    }
